...
@st.cache_resource
def load_classifier():
    tokenizer = AutoTokenizer.from_pretrained("models/audit_distilbeto")
    model = AutoModelForSequenceClassification.from_pretrained("models/audit_distilbeto")
    ...
    if extracted_text.strip() != "":
        ...
        if prob_recomend >= threshold:
            if st.button("Generar actividades con OpenAI"):
                # Configura tu API key en st.secrets["OPENAI_API_KEY"]
